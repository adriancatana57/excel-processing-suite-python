...
def normalize_value(value):
    if value is None:
        return ""
    s = str(value)
    # Remove control and format characters (invisible)
    s = ''.join(c for c in s if unicodedata.category(c) not in ('Cc', 'Cf'))
    return s

def is_blank(value):
    """Returns True if the value is empty or contains only whitespace or invisible characters."""
...
def build_reference_map(ref_path, ref_key_col, data_cols, ref_sep=None, ref_encoding=None,
    ...
    if filetype in ("csv", "txt"):
        ...
        for row in iter_text_rows(ref_path, sep, enc, skip_header=True):
            key = normalize_value(row[key_idx] if key_idx < len(row) else "")
            if allow_repeats:
                d = ref_map.get(key)
                if d is None:
                ...
                for c, ix in zip(data_cols, data_idx):
                    val = normalize_value(row[ix] if ix < len(row) else "")
                    if not is_blank(val):
                        d[c].append(val)
                ...
                vals = {}
                for c, ix in zip(data_cols, data_idx):
                    vals[c] = normalize_value(row[ix] if ix < len(row) else "")
                ref_map[key] = vals
...
def stream_join_and_write(princ_path, ref_map, princ_key_col, selected_princ_cols, data_cols,
    ...
    if filetype in ("csv", "txt"):
        ...
        rows = list(iter_text_rows(princ_path, sep, enc, skip_header=True))
        rows_iter = iter(rows)
        ...
        rows = list(iter_xlsx_rows(princ_path, sheet_name=princ_sheet, skip_header=True))
        rows_iter = iter(rows)
        out_sep = TEXT_SEP_DEFAULT_TXT if out_format == ".txt" else TEXT_SEP_DEFAULT_CSV
    else:
    ...
    # Calculate actual max_len_per_col based on matched rows
    actual_max_len = {c: 0 for c in data_cols}
    for row in rows:
        key = normalize_value(row[princ_key_idx] if princ_key_idx < len(row) else "")
        match_vals = ref_map.get(key)
        if match_vals:
            for col in data_cols:
                lst = match_vals.get(col, [])
                actual_max_len[col] = max(actual_max_len[col], len(lst))

    # Filter data_cols to only include those with actual_max_len > 0
    filtered_data_cols = [c for c in data_cols if actual_max_len[c] > 0]
    max_len_per_col = {c: actual_max_len[c] for c in filtered_data_cols}

    imported_col_names = []
    if allow_repeats:
        for col in filtered_data_cols:
            m = max_len_per_col.get(col, 1)
            for i in range(1, m + 1):
                imported_col_names.append(col if i == 1 else f"{col}_{i}")
    else:
        imported_col_names.extend(filtered_data_cols)
...
class App(ctk.CTk):
    ...
    def on_preview(self):
        ...
        try:
            ...
            # Calculate actual max_len based on preview rows
            actual_max_len = {c: 0 for c in self.data_cols_selected}
            pkey_idx = headers.index(self.main_key.get())
            for r in rows:
                key = normalize_value(r[pkey_idx] if pkey_idx < len(r) else "")
                match_vals = ref_map.get(key)
                if match_vals:
            ...
            # Filter data_cols_selected
            filtered_data_cols = [c for c in self.data_cols_selected if actual_max_len[c] > 0]
            max_len_per_col = {c: actual_max_len[c] for c in filtered_data_cols}
            ...
            if allow_repeats:
                ...
                imported.extend(filtered_data_cols)
            ...
            for r in rows:
                ...
                if match_vals is None:
                    ...
                    if allow_repeats:
                        ...
                        imp_vals.extend([""] * len(filtered_data_cols))
                else:
                    imp_vals = []
                    if allow_repeats:
                        for col in filtered_data_cols:
